...
class ApiController extends BaseController
{
     ...
     * @var array
     */
    protected $allowedFields = array('search' => array(), 'partials' => array());
    ...
    protected function parseRequest($allowedFields) {
        ...
        if ($searchParams) {
            ...
            if (array_diff(array_keys($this->searchFields), $this->allowedFields['search'])) {
                throw new HTTPException("The fields you specified cannot be searched.", 401, array('dev' => 'You requested to search fields that are not available to be searched.', 'internalCode' => 'S1000', 'more' => ''
     ...
     */
    protected function sendRespond($response) {
        $result = new \Base\Framework\Responses\Response();
        $result->model = ($response->model != null && gettype($response->model) == 'object' ? $this->object2array($response->model) : $response->model);
        $result->messages = ($response->messages != null && gettype($response->messages) == 'object' ? $response->messages->toArray() : $response->messages);
        return $result;
    }
     ...
     * @return array result
     */
    protected function TraverseResult($objects) {
        $result = array();
    ...
    protected function mapper($model, $entity) {
        foreach (get_object_vars($model) as $property => $value) {
            if (isset($entity[$property])) {
                if ($property == 'password') {
                    $model->$property = $this->security->hash($entity[$property]);
                } 
                else {
